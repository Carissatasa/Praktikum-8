...
<body>
	...
	if ($_SERVER["REQUEST_METHOD"] == "POST") {
		...
		if (empty($_POST["web"])) {
			...
			if (!preg_match("/\b(?:(?:https?|ftp):\/\/\www\. ) [−a-z0−9+&@#\/%?=~_| ! :, • ;]*[−a−z®−9+&@#\/%~~_|]/i", $web)) {
		...
		if (empty($_POST["telp"])) {
			...
			if(!is_numeric($telp)) {
				$error_telp = 'Nomor HO hanya boleh angka';
			}
		}
			...
			<div class="card-body">
				<form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
					...
					<div class="form-group row">
						...
						<div class="col-sm-10">
							<input type="text" name="web" class="form-control <?php echo ($error_web !="" ? "is-invalid" : ""); ?>" id="nama" placeholder="Web" value="<?php echo $web; ?>"><span class="warning"><?php echo $error_web; ?></span>
						</div>
					</div>
